...
/**
 * Returns TRUE when the current request is a POST from our own settings form.
 *
 * Used as an early-exit guard before accessing $_POST fields.
 *
 * @return bool
 */
function rwa_lite_is_our_settings_post() {
	...
	// We only read 'option_page' to identify our own form before the nonce check.
	// sanitize_key + wp_unslash are applied so PHPCS does not flag raw $_POST access.
	// The actual nonce verification occurs immediately after in rwa_lite_sanitize_agents().
	$option_page = isset( $_POST['option_page'] ) // phpcs:ignore WordPress.Security.NonceVerification.Missing
		...
		: '';
	return 'rwa-lite-group' === $option_page;
}
...
function rwa_lite_sanitize_agents( $input ) {
	...
	// ── Nonce verification ────────────────────────────────────────────────────
	// check_admin_referer() verifies the nonce and calls wp_die() on failure,
	// protecting against Cross-Site Request Forgery (CSRF) attacks.
	if ( rwa_lite_is_our_settings_post() ) {
...
/**
 * Renders the floating WhatsApp button and the agent-selection script
 * in the page footer.
 *
